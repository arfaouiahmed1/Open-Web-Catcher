...
These diagrams describe the runtime as it exists today, grounded in the source files listed under each diagram. They predate the target architecture: where this snapshot and target-design.md disagree, the target design wins and the change is tracked in `.omo/plans/full-audit.md`. Moved here from `docs/README.md` when that file became a reading guide (plan task 46).
