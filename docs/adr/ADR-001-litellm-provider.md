...
- `normalize_model_name` maps legacy bare names such as `gemini-2.5-flash` to LiteLLM's `provider/model` routing format, using the `llm_provider` setting as the prefix hint. Unrecognized names pass through bare so misconfiguration surfaces at the provider instead of being masked.
- Provider switching is config-only: `llm_provider`, `llm_base_url`, and the existing per-agent model map. No code changes to move families.
...
- Any LiteLLM-supported provider is reachable by editing settings.
