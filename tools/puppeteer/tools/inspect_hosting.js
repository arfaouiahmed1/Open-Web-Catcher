...
export async function inspectHosting(params = {}) {
  ...
  const rootTargets = [
    ...
    ...(data.elements || []).map((entry) =>
      normalizeTarget(entry, entry.frame_path || "root"),
    ),
  ];

  const frameTargets = (data.frame_tree || []).flatMap((frame) => [
    ...(frame.sample_buttons || []).map((entry) =>
      normalizeTarget(entry, frame.frame_path),
    ),
  ...
  const server_controls = dedupeBy(
    allTargets
      ...
      .sort(
        (a, b) =>
          (b.frame_path === "root" ? 0 : 1) - (a.frame_path === "root" ? 0 : 1),
      ),
  ...
  const playback_targets = dedupeBy(
    [
      ...
      ...allTargets.filter((entry) =>
        PLAY_PATTERN.test(`${entry.text} ${entry.selector} ${entry.xpath}`),
      ),
    ],
    (entry) => `${entry.frame_path}|${entry.selector}|${entry.xpath}|${entry.text}`,
  );
  ...
  return {
    ...
    iframe_context: {
      ...
      frames_with_server_controls: (data.frame_tree || []).filter(
