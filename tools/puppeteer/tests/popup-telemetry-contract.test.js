...
test("tab tracker adopts same-content player and closes ad targets", async () => {
  ...
  const tracker = trackNewTabs(browser, { openerPage: opener, adopt: true, closeUnadopted: true });

  browser.emit("targetcreated", fakeTarget(adPage));
  browser.emit("targetcreated", fakeTarget(playerPage));
...
test("tab tracker exposes decoded player urls from adopted popup redirects", async () => {
  ...
  browser.emit("targetcreated", fakeTarget(popupPage));
  const activePage = await tracker.settle({ timeoutMs: 50 });
  tracker.dispose();
