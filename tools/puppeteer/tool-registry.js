...
const TOOL_SPECS = {
  ...
  play_media: spec(
    "Use when the agent has chosen an exact player/video/play target. With selector/xpath/text/element_ref or explicit x/y, activates only that chosen target plus existing video playback and verifies media state. If called bare, returns activation_candidates and needs_agent_choice without clicking guessed controls.",
    { frame_path: "root.0", selector: "video", wait_ms: 1500 },
    { ok: true, screenshot_url: "https://res.cloudinary.com/...", playback_started: true, verification_signal: "playing", frame_relocated: false, needs_agent_choice: false },
    {
      frame_path: framePathSchema,
      element_ref: elementRefSchema,
      selector: selectorSchema,
  ...
  inspect_hosting: spec(
    "Hosting-page inspect optimized for LLM-chosen activation and server switching. Returns server_frontier, event_server_routes, activation_candidates, blocker_candidates, control_groups, playback_groups, iframe_groups with iframe-local sample_buttons/sample_links/sample_videos, player_evidence, top_server_controls, top_playback_targets, popups, lazy_load_warmup, and compression-aware stats after top-reset scrolling. Use server_frontier as the initial queue and choose exact activation/blocker targets from returned evidence.",
    { scroll: true, scroll_steps: 12 },
    {
      ...
      server_frontier: [{ label: "Server 1", action: "interact", selector: ".server-btn", frame_path: "root" }],
      event_server_routes: [{ label: "Admin Stream 1", source_url: "https://example.com/watch/1/admin/1", action: "navigate" }],
      ...
      activation_candidates: [{ kind: "video", selector: "video", x: 960, y: 540, reason: "video element", requires_agent_choice: true }],
      blocker_candidates: [{ text: "Close Ad", close_selector: ".close", reason: "popup close control" }],
  ...
  inspect_embedded: spec(
    "Embedded-page inspect optimized for nested player routing and LLM-chosen activation. Returns activation_candidates, blocker_candidates, control_groups, player_groups, frame_focus_groups, player_evidence, top_source_controls, top_player_targets, popups, lazy_load_warmup, and compression-aware stats after top-reset scrolling.",
    { scroll: true, scroll_steps: 12 },
    {
      ...
      activation_candidates: [{ kind: "video", selector: "video", frame_path: "root.0", reason: "video element", requires_agent_choice: true }],
      blocker_candidates: [{ text: "Dismiss", close_selector: ".close", reason: "popup close control" }],
      player_evidence: { has_video: true, has_player_iframe: true, server_tabs: true },
      lazy_load_warmup: { scroll_steps: 12, reset_to_top: true },
      screenshot_url: "https://res.cloudinary.com/...",
    },
