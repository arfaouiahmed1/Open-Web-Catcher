...
import { connectBrowser, getPage, setActivePage } from './browser.js';
...
export function makeObservedChange(before, after, newTabUrls = [], popupTelemetry = {}) {
  return {
    ...
    target_decision: popupTelemetry.target_decision || (newTabUrls.length ? 'no_adoptable_popup' : 'no_popup'),
...
export function trackNewTabs(browser, {
    ...
    async settle({ timeoutMs = 3000 } = {}) {
      ...
      tracker.target_decision = tracker.selected_target
        ...
        : (tracker.blocked_popup_attempts.length
          ...
          : (tracker.opened_targets.length ? 'no_adoptable_popup' : 'no_popup'));
      tracker.active_page_url = resultPage?.url?.() || openerUrl;
      setActivePage(browser, resultPage || openerPage);
  ...
  const recordTarget = async (target) => {
    ...
    try {
      ...
      candidate.final_url = candidate.url;
      candidate.title = await popup.title().catch(() => '');
      newTabUrls[candidate.index] = candidate.url;
    } catch {
      // ignore
    }
