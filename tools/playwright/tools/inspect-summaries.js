...
function buildOutputCounts(payload) {
  return {
    ...
    // Playwright inspect-family payload keys (additive; plan T20-d).
    match_candidates: (payload.match_candidates || []).length,
    navigation_links: (payload.navigation_links || []).length,
    action_targets: (payload.action_targets || []).length,
    server_controls: (payload.server_controls || []).length,
    playback_targets: (payload.playback_targets || []).length,
    source_controls: (payload.source_controls || []).length,
    player_targets: (payload.player_targets || []).length,
    frame_focus_order: (payload.frame_focus_order || []).length,
    iframe_frames: (payload.iframe_overview?.frames || payload.iframe_context?.frames || []).length,
  };
}
...
export {
  ...
  jsonBytes,
  // Additive exports (plan T20-d): budget machinery reused by the playwright
  // inspect family (inspect_landing/hosting/embedded).
  PRIORITY_RANK,
  fitPayloadToBudget,
  attachCompressionTelemetry,
  buildRawCounts,
  commonReducers,
  reduceArray,
  reduceSamples,
  dropLowPriorityGroups,
};
