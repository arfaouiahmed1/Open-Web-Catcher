...
class ToolResultCache:
    ...
    def get(self, tool_name: str, tool_args: dict[str, Any]) -> str | None:
        """Return cached result string if stable, else None."""
        key = _tool_cache_key(tool_name, tool_args)
        entry = self._store.get(key)
        if (
            entry is not None
            and entry.get("cached_result")
            and int(entry.get("stable_observations", 0)) >= self._min_obs
        ):
            self.hits += 1
            return str(entry["cached_result"])
        return None
