...
function LookupRow({ row, even }) {
  return (
    <tr style={{ background: even ? "rgba(255,255,255,0.012)" : "transparent" }}>
      ...
      <td className="px-4 py-2.5 text-[12px]" style={{ color: "var(--mute)" }}>
...
export default function ProvidersPage() {
  ...
  useEffect(() => {
    loadHistory().catch((loadError) => setError(loadError.message));
  }, []);
              ...
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--line)", background: "rgba(255,255,255,0.012)" }}>
                  {["URL", "IP", "Provider", "Org", "Country", "Abuse contact"].map((header) => (
                    <th key={header} className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-[0.12em] whitespace-nowrap" style={{ color: "var(--mute)" }}>{header}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
