...
export default function Error({
  ...
  React.useEffect(() => {
    // Log for observability; Next.js already reports to the console, but
    // this keeps a uniform prefix for filtering.
    // eslint-disable-next-line no-console
    console.error("[app/error]", error);
  }, [error]);

  const message = error?.message?.trim() ? error.message.trim() : "An unexpected error occurred.";
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <CardTitle className="text-base font-semibold leading-tight">Something went wrong</CardTitle>
              <CardDescription className="mt-1.5 text-sm leading-relaxed">
                The console ran into an unexpected error. You can reset the view or return to the dashboard.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
