...
function Metric({ label, value, accent, detail }) {
  return (
    <div className="flex items-baseline gap-2">
      <span className="font-mono text-[16px] font-semibold" style={{ color: accent }}>{value}</span>
      <span className="text-[10px] uppercase tracking-[0.12em]" style={{ color: "var(--mute-2)" }}>{label}</span>
      {detail ? <span className="font-mono text-[10px]" style={{ color: "var(--mute-3)" }}>{detail}</span> : null}
    </div>
  );
}
...
function TabButton({ active, onClick, children }) {
  return (
    <button
      ...
      style={{
        borderColor: active ? "color-mix(in oklch, var(--signal) 36%, transparent)" : "var(--line)",
...
export function RunDetailLive({ runId, activeTrace = null, persistedEvents = [], metrics = null, onMetricsChange = null }) {
  ...
  async function cancelRun() {
    ...
    try {
      ...
      setCancelError(error instanceof Error ? error.message : String(error || "Cancel failed"));
    } finally {
  ...
  const cachedInputTokens = Number(effectiveMetrics?.total_cached_input_tokens || 0);
  ...
  const toolCalls = Math.max(toolCallRows.length, Number(effectiveMetrics?.total_tool_calls || 0));
  const llmCalls = Number(effectiveMetrics?.total_llm_calls || 0);
  const rootActor = activeTrace?.root_actor || events.find((event) => event?.actor)?.actor || "orchestrator";
    ...
    <div className="space-y-4">
      ...
      >
        <button
          ...
          style={{
            borderColor: liveStream ? "color-mix(in oklch, var(--rose) 35%, transparent)" : "var(--line)",
            background: liveStream ? "color-mix(in oklch, var(--rose) 12%, transparent)" : "transparent",
            color: liveStream ? "var(--rose)" : "var(--mute-2)",
          }}
        >
          {liveStream ? <PauseCircle className="h-4 w-4" /> : <PlayCircle className="h-4 w-4" />}
        ...
        >
          {isReplaying ? <Loader2 className="h-4 w-4 animate-spin" /> : <TimerReset className="h-4 w-4" />}
        ...
        <input
          ...
          className="w-16 rounded-lg border px-2 py-1 text-[11px]"
          style={{ borderColor: "var(--line)", background: "rgba(0,0,0,0.12)", color: "var(--ink-dim)" }}
        />
        <span className="font-mono text-[10px]" style={{ color: "var(--mute-3)" }}>ms</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <TabButton active={tab === "tools"} onClick={() => setTab("tools")}>Tool calls</TabButton>
        </div>
      </div>
      ...
      {effectiveMetrics ? (
        ...
        >
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <Metric label="Tools" value={formatNumber(toolCalls)} accent="var(--sky)" />
            <Metric label="LLM" value={formatNumber(llmCalls)} accent="var(--violet)" />
            <Metric label="Tokens" value={formatNumber(totalTokens)} accent="var(--signal)" detail={formatNumber(totalInputTokens)} />
            <Metric
              label="Cached / input"
            ...
            <Metric
              label="Cost"
              value={formatCurrency(effectiveMetrics.total_cost_usd ?? effectiveMetrics.estimated_total_cost_usd ?? 0)}
              accent="var(--mint)"
            />
          </div>
        </div>
      ...
      {runId ? <BrowserLiveView runId={runId} events={events} autoRefresh={isLive} /> : null}
