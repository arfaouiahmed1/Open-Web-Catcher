...
function Bar({ label, value, total, color }) {
  ...
  return (
    <div
      className="flex flex-col gap-1.5 rounded-[14px] border px-3 py-2.5"
      style={{
...
export function CostEstimateCard({
  ...
  const effectiveCalls = useMemo(() => {
    ...
    return synthCallsFromModelUsage(modelUsage);
  }, [llmCalls, modelUsage]);

  const totals = useMemo(
    () => estimateRunCost(effectiveCalls, pricingMap),
    [effectiveCalls, pricingMap],
  );

  const coverage =
    totals.calls > 0 ? Math.round((totals.computed / totals.calls) * 100) : 0;
  const coverageLabel =
    coverage >= 100 ? "Fully priced" : coverage > 0 ? "Partially priced" : "Pricing missing";
  ...
  if (unavailable) {
    return (
      <Card className="overflow-hidden shadow-card">
        <CardHeader
          className="flex flex-row items-start justify-between gap-3 p-4"
          style={{
            background:
              "linear-gradient(180deg, color-mix(in oklch, var(--signal) 8%, transparent), transparent 72%)",
          }}
        >
          <div>
            ...
            <div className="mt-1 font-mono text-[31px] font-semibold leading-none" style={{ color: "var(--signal)" }}>
              --
            </div>
            <p className="mt-2 text-[12px]" style={{ color: "var(--mute)" }}>
              No persisted LLM telemetry was available for this run.
            </p>
          </div>
          <div className="flex flex-col items-end gap-2 text-right text-[10px] text-muted-foreground">
            <span
              className="rounded-full border px-2.5 py-1 font-medium uppercase tracking-[0.12em]"
              style={{
                borderColor: "var(--line)",
                color: "var(--signal)",
        ...
        <CardContent className="border-t p-4 text-[12px] text-muted-foreground" style={{ borderColor: "var(--line)" }}>
  ...
  return (
    <Card className="overflow-hidden shadow-card">
      <CardHeader
        ...
        style={{
          background:
            "linear-gradient(180deg, color-mix(in oklch, var(--mint) 8%, transparent), transparent 72%)",
        }}
      >
        <div>
          <CardTitle className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
          ...
          <div className="mt-1 font-mono text-[31px] font-semibold leading-none" style={{ color: "var(--mint)" }}>
            {formatCurrency(totals.total)}
          </div>
          <p className="mt-2 text-[12px]" style={{ color: "var(--mute)" }}>
            New input, cached reads, cache writes, and output are tracked separately.
          </p>
        </div>
        <div className="flex flex-col items-end gap-2 text-right text-[10px] text-muted-foreground">
          <span
            ...
            style={{
              borderColor: "var(--line)",
              color: coverage >= 100 ? "var(--mint)" : coverage > 0 ? "var(--signal)" : "var(--rose)",
              background:
                coverage >= 100
      ...
      {pricingMap && pricingMap.size === 0 ? (
        <div
          className="mx-4 mb-4 rounded-lg px-2.5 py-1.5 text-[10.5px]"
          style={{
      ...
      {pricingMap && pricingMap.size > 0 && totals.unpriced > 0 ? (
        <div
          className="mx-4 mb-4 rounded-lg px-2.5 py-1.5 text-[10.5px]"
          style={{
            background: "color-mix(in oklch, var(--signal) 10%, transparent)",
        ...
        >
          Pricing missing for {formatNumber(totals.unpriced)} provider/model call{totals.unpriced === 1 ? "" : "s"}.
        </div>
      ) : null}
    </Card>
  );
